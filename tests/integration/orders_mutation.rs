...
fn gtd_unsupported(err: &polyester::Error) -> bool {
    ...
    .iter()
    .any(|token| msg.contains(token))
}

#[tokio::test]
async fn gtd_limit_order_round_trip() {
    if !require_account_wide_cleanup() {
        return;
    }
    if !require_mutation() {
    ...
    let scale = client
        ...
        .base_quantity_scale_for_symbol(&symbol)
        .expect("catalog scale");
    let client_order_id = unique_client_order_id("gtd");
    let params = CreateOrderParams {
        ...
        time_in_force: Some(CreateTimeInForce::Gtd),
        client_order_id: Some(client_order_id.clone()),
        subaccount_id: None,
        post_only: Some(true),
        market_client_ref_price: None,
    ...
    let created = match client.orders.create(params).await {
        ...
        Err(err)
            if is_internal_order_error(&err)
                || devnet_unavailable(&err)
                || gtd_unsupported(&err) =>
        {
            eprintln!("skip: devnet GTD order placement unavailable: {err}");
            return;
        }
        Err(err) if is_notional_validation(&err) => {
            eprintln!("skip: order sizing below min notional: {err}");
        ...
        Err(err) => panic!("orders.create gtd: {err}"),
    };
    assert_eq!(created.client_order_id, client_order_id);
    ...
    let open_order = match wait_for_open_order(&client, &client_order_id, Duration::ZERO).await {
        Ok(o) => o,
        Err(err) if err.downcast_ref::<DevnetOrderNotIndexedError>().is_some() => {
            eprintln!("skip: GTD order accepted but not indexed");
            let _ = client.orders.cancel_all(Some(&symbol), false, None).await;
            return;
        }
        Err(err) => panic!("wait open: {err}"),
    };
    assert_eq!(open_order.tif, "gtd");
    assert!(
        open_order
            .expire_at
            .as_ref()
            .is_some_and(|value| !value.is_empty()),
        "expected expire_at on GTD open order"
    );
    ...
    assert!(order.expire_at.is_some());

    let _ = client
        .orders
        .cancel_by_client_order_id(&client_order_id, Some(&symbol), None)
        .await;
    let _ = wait_for_no_open_order(&client, &client_order_id, Duration::ZERO).await;
    let _ = client.orders.cancel_all(Some(&symbol), false, None).await;
}
