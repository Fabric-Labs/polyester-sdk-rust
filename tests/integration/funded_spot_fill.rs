use crate::support::{
    ...
    pair_for_symbol, quote_asset_id, require_funded, require_live_client,
    require_trading_quote_balance, trade_e2e_enabled, trade_symbol, trading_balance_human,
...
async fn spot_fill() {
    ...
    if !trade_e2e_enabled() {
        eprintln!("skip: Set POLYESTER_TEST_TRADE_E2E=1 to run spot fill e2e");
        return;
    }
    ...
    if taker_quote < required_quote {
        eprintln!(
            "skip: taker quote balance {taker_quote} below required {required_quote} for {qty} {symbol} at {price}"
        );
        return;
    }
    ...
    match crate::support::wait_for_terminal_order(&client, &taker_cid, Duration::from_secs(20))
        .await
    {
        Ok(_) => {}
        Err(err) => {
            eprintln!("skip: wait fill: {err}");
        }
    }
