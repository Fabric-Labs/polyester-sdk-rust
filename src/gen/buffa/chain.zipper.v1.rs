...
pub mod __buffa {
    ...
    pub mod view {
        ...
        /** Self-contained, `'static` owned view of a `ZipperReasonDetails` message.

 Wraps [`::buffa::OwnedView`]`<`[`ZipperReasonDetailsView`]`<'static>>`: the decoded view and the [`::buffa::bytes::Bytes`] buffer it borrows from travel together, so the handle is `'static` and `Send + Sync` — suitable for async handlers, spawned tasks, and anywhere a `'static` bound is required.

 Field accessors return borrows tied to `&self`. Use [`Self::view`] to get the full [`ZipperReasonDetailsView`] when you need struct patterns, iteration helpers, or to pass the view to lifetime-parameterised code.*/
        #[derive(Clone, Debug)]
        ...
        impl ZipperReasonDetailsOwnedView {
            ...
            /// Borrow the full [`ZipperReasonDetailsView`] with its lifetime tied to `&self`.
            #[must_use]
